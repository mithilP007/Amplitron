...
// ============================================================
// MAIN RENDER LOOP — ADDED MIDI PROCESSING
// ============================================================
void GuiManager::render() {
    if (!initialized_) return;

    // Process MIDI messages from hardware controllers
    // This must happen before ImGui frame to ensure MIDI state is current
    midi_manager_.process_messages();
    ...
    // (menu bar, pedal board, settings, etc.)

    // Example: Add MIDI menu to menu bar if you have one
    /*
    if (ImGui::BeginMainMenuBar()) {
        ...
        // MIDI Menu — shows current status
        if (ImGui::BeginMenu("MIDI")) {
            if (midi_manager_.is_initialized()) {
                ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "● MIDI Active");
                ImGui::Text("Device: %s", midi_manager_.current_device_name().c_str());
                ImGui::Text("Mappings: %zu", midi_manager_.mappings().size());
                
                ImGui::Separator();
                
                if (ImGui::MenuItem("Re-scan Devices")) {
                    midi_manager_.rescan_devices();
                }
                if (ImGui::MenuItem("Clear All Mappings")) {
                    midi_manager_.clear_mappings();
                }
            } else {
                ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "● MIDI Inactive");
                ImGui::TextDisabled("No MIDI device connected");
            }
            ImGui::EndMenu();
        }
    ...
    */
    ...
    // Render pedal board (which includes MIDI learn via right-click)
    if (pedal_board_) {
        pedal_board_->render();
    }

    // ... existing render code for other panels ...
