...
void GuiSettings::render(bool& show) {
    ...
    if (!midi) {
        ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.0f, 1.0f), 
            "MIDI support not available in this build.");
        ImGui::TextWrapped("Build with RtMidi to enable MIDI control.");
    } else {
        // MIDI Device Selection
        ImGui::Text("MIDI Input Device:");
        
        auto ports = midi->get_available_ports();
        static int selected_midi_port = -1;
        
        std::string preview = selected_midi_port >= 0 && selected_midi_port < (int)ports.size() 
            ? ports[selected_midi_port] 
            : "Select MIDI device...";
        
        if (ImGui::BeginCombo("##MidiDevice", preview.c_str())) {
            // "None" option to close MIDI
            bool is_none_selected = (selected_midi_port == -1);
            if (ImGui::Selectable("None (Disable MIDI)", is_none_selected)) {
                selected_midi_port = -1;
                midi->close();
            }
            ...
            for (int i = 0; i < (int)ports.size(); i++) {
                bool is_selected = (selected_midi_port == i);
                if (ImGui::Selectable(ports[i].c_str(), is_selected)) {
                    selected_midi_port = i;
                    if (!midi->open(i)) {
                        selected_midi_port = -1;
                    }
                }
        ...
        if (midi->is_learning()) {
            ...
            ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), 
                "⚡ MIDI LEARN ACTIVE — Move a controller now!");
            ImGui::SameLine();
        ...
        // Active MIDI Mappings table
        auto mappings = midi->get_mappings();
        if (!mappings.empty()) {
            ImGui::Spacing();
            ImGui::Text("Active MIDI Mappings:");
            
            if (ImGui::BeginTable("MidiMappings", 5, 
                ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingFixedFit)) {
                
                ImGui::TableSetupColumn("CC#", ImGuiTableColumnFlags_WidthFixed, 50);
                ImGui::TableSetupColumn("Effect", ImGuiTableColumnFlags_WidthFixed, 60);
                ...
                for (size_t i = 0; i < mappings.size(); i++) {
                    ...
                    ImGui::TableNextColumn();
                    ImGui::Text("%d", m.effect_index);
                    
                    ImGui::TableNextColumn();
                    ImGui::Text("%d", m.parameter_index);
                    
                    ImGui::TableNextColumn();
                    ImGui::Text("%s", m.is_toggle ? "Toggle" : "Range");
                    
                    ImGui::TableNextColumn();
                    std::string btn_id = "Remove##" + std::to_string(i);
                    if (ImGui::SmallButton(btn_id.c_str())) {
                        midi->remove_mapping(m.cc_number);
                    }
                }
                ImGui::EndTable();
            }
            ...
            if (ImGui::Button("Save Mappings")) {
                if (midi->save_mappings("midi_mappings.json")) {
            ...
            ImGui::TextDisabled("No MIDI mappings. Right-click any pedal knob to add.");
        }
    }
